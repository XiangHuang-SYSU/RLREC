...
class WLRRDM(nn.Module):
    ...
    def forward(self, img, x1=None, x2=None):
        ...
        if self.training:
            ...
            IPR = x_noisy
            cc = self.condition(img)
            cc2 = self.condition2(img)
            #cc3= self.condition(img)
            c = torch.cat([cc,cc2],dim=1)
            
            #print("cc.shape",cc.shape) #torch.Size([1, 3200])
            

            c0 = ptwt.wavedec(c, pywt.Wavelet("haar"),level=1, mode="constant")
            c = torch.cat([c0[0],c0[1]],dim=1)
            
            for i in reversed(range(0, self.num_timesteps)):
                IPR, predicted_noise = self.p_sample(IPR, torch.full((b,), i,  device=device, dtype=torch.long), c,
                                clip_denoised=self.clip_denoised)
                
                IPR1,IPR2 = IPR.chunk(2, dim=1)
                IPR = ptwt.waverec([IPR1,IPR2], pywt.Wavelet("haar"))
