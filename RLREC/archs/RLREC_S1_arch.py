...
class dino_S(nn.Module):
    def __init__(self, n_feats = 64, DINO_REPO_DIR=None):
        ...
        self.dino_mlp = nn.Sequential(
            nn.Linear(n_feats * 24 * 6, n_feats * 8),
            nn.LeakyReLU(0.1, True),
            nn.Linear(n_feats * 8, n_feats * 4),
            nn.LeakyReLU(0.1, True),
            nn.Linear(n_feats * 4, n_feats * 4),
    ...
    def forward(self, img,gt_img):
        x = self.resize_transform(img)
        x_feats = self.dinov3_vitb16.get_intermediate_layers(x, n=range(12), reshape=True, norm=True)
        gt = self.resize_transform(gt_img)
        gt_feats = self.dinov3_vitb16.get_intermediate_layers(gt, n=range(12), reshape=True, norm=True)
        x_feats_last = torch.cat([x_feats[6],gt_feats[6],x_feats[7],gt_feats[7],x_feats[8],gt_feats[8],x_feats[9],gt_feats[9],x_feats[10],gt_feats[10],x_feats[11],gt_feats[11],],dim=1)
        x_feats_last = self.dino_mlp_2(x_feats_last.detach())
        x_feats_last = self.dino_mlp_2(x_feats_last.squeeze(-1))
        output = self.dino_mlp(x_feats_last.squeeze(-1))
        return output
...
class RLRECS1(nn.Module):
    def __init__(self, 
        ...
        self.Edino_A = dino_A(n_feats=64, DINO_REPO_DIR=DINO_REPO_DIR)
        self.Edino_S = dino_S(n_feats=64, DINO_REPO_DIR=DINO_REPO_DIR)
        
        self.padding_width = nn.ConstantPad2d((0,1),0)
        self.padding_height = nn.ConstantPad2d((0,0,1,0),0)

    def forward(self, x, gt, lq_Structure_gt_Appearance, lq_Appearance_gt_Structure):
        if self.training:
            lq_S_gt_A_fea = self.Edino_A(lq_Structure_gt_Appearance,gt)
            lq_A_gt_S_fea = self.Edino_S(lq_Appearance_gt_Structure,gt)
            sr = self.G(x, lq_S_gt_A_fea, lq_A_gt_S_fea)
            return sr, lq_S_gt_A_fea, lq_A_gt_S_fea
        else:
            padding_num_h = 0
            padding_num_w = 0
            n,c,h,w = x.shape
            while (w/4)%2 != 0:
                x = self.padding_width(x)
                gt = self.padding_width(gt)
                padding_num_w = padding_num_w + 1
                w = w + 1
            while (h/4)%2 != 0:
                x = self.padding_height(x)
                gt = self.padding_height(gt)
